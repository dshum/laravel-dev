...
Route::group(array('before' => 'admin.auth'), function() {
	...
	Route::get('/admin/order/{class}/{classId?}', array('as' => 'admin.order', function($class, $classId = null) {
		try {
			$site = App::make('site');
			$item = $site->getItemByName($class);
			$element = LemonTree\Element::getByClassId($classId);
			return App::make('LemonTree\OrderController')->getIndex($item, $element);
		} catch (Exception $e) {
			echo $e->getTraceAsString(); die();
			return Redirect::route('admin');
		}
	...
	Route::get('/admin/service', array('as' => 'service', 'uses' => 'LemonTree\ServiceController@getService'));
});
...
Route::group(array('before' => 'admin.auth.post'), function() {
	...
	Route::post('/admin/browse/{classId}/post/{method}', array('as' => 'admin.browse.post', function($classId, $method) {
		$loggedUser = Sentry::getUser();
		$element = LemonTree\Element::getByClassId($classId);

		if (
			! $element
			|| ! $loggedUser->hasViewAccess($element)
		) {
			return Redirect::route('admin');
		}
		...
		if ($browsePlugin) {
			return App::make($browsePlugin)->$method($element);
		}

		return Redirect::route('admin.browse', $classId);
	}));

	Route::post('/admin/search/{class}/post/{method}', array('as' => 'admin.search.post', function($class, $method) {
		$loggedUser = Sentry::getUser();
		...
		$searchPlugin = App::make('site')->getSearchPlugin($class);

		if ($searchPlugin) {
			return App::make($searchPlugin)->$method($item);
		}

		return Redirect::route('admin.search');
	}));

	Route::post('/admin/edit/{classId}/post/{method}', array('as' => 'admin.edit.post', function($classId, $method) {
		$loggedUser = Sentry::getUser();
		$element = LemonTree\Element::getByClassId($classId);
		...
		$editPlugin = App::make('site')->getEditPlugin($classId);

		if ($editPlugin) {
			return \App::make($editPlugin)->$method($element);
		}

		return Redirect::route('admin.edit', $classId);
	}));
...
Route::group(array('before' => 'admin.auth.ajax'), function() {
	...
	Route::post('/admin/order/save/{class}', array('as' => 'admin.order.save', 'uses' => 'LemonTree\OrderController@postSave'));
	...
	Route::post('/admin/browse/{classId}/ajax/{method}', array('as' => 'admin.browse.ajax', function($classId, $method) {
		$loggedUser = Sentry::getUser();
		$element = LemonTree\Element::getByClassId($classId);
		...
		$scope['redirect'] = URL::route('admin.browse', $classId);
		return json_encode($scope);
	}));

	Route::post('/admin/search/{class}/ajax/{method}', array('as' => 'admin.search.ajax', function($class, $method) {
		$loggedUser = Sentry::getUser();
		...
		if (
			...
			$scope['redirect'] = URL::route('admin.search');
			return json_encode($scope);
		}
	...
	Route::post('/admin/edit/{classId}/ajax/{method}', array('as' => 'admin.edit.ajax', function($classId, $method) {
		$loggedUser = Sentry::getUser();
		$element = LemonTree\Element::getByClassId($classId);
		...
		$scope['redirect'] = URL::route('admin.edit', $classId);
		return json_encode($scope);
	}));
