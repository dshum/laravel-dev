...
class SearchController extends BaseController {
	...
	public function getIndex()
	{
		...
		$searchPluginView = null;

		$searchPlugin = $site->getSearchPlugin($currentItem->getName());

		if ($searchPlugin) {
			try {
				$view = \App::make($searchPlugin)->getIndex($currentItem);
				if ($view) {
					$searchPluginView = is_string($view)
						? $view : $view->render();
				}
			} catch (\Exception $e) {
				$searchPluginView = nl2br($e->getTraceAsString());
			}
		}

		\View::share('searchPluginView', $searchPluginView);
